...
hbca_epi <- readRDS(file.path(PATH, "brca_atlas_validation/data/sigs/all_markers.rds"))
hbca_epi <- hbca_epi[1:11]

RNGkind("L'Ecuyer-CMRG")
set.seed(1)
K2res <- K2preproc(object = t(epi_subset@reductions$integrated.rpca@cell.embeddings),
                   eMatDS = epi_subset@assays$RNA$data,
                   colData = epi_subset@meta.data,
                   variables = "batch",
                   cohorts="RNA_snn_res.0.1",
                   featMetric="F",
                   logCounts=TRUE,
...
K2dashboard(K2res,  analysis_name = "K2Taxonomer_Epi")
