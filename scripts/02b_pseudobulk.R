...
# Loading Data
combined_seurat <- readRDS(file=file.path(PATH, "data/sc/combined_seurat.rds"))

# Averaging by donor (sample)
# combined_data_avg <- Seurat::AggregateExpression(combined_seurat, group.by="donor", return.seurat=TRUE)
combined_data_avg <- Seurat::AggregateExpression(combined_seurat, group.by="donor", normalization.method = "LogNormalize", return.seurat=TRUE)
...
saveRDS(combined_data_avg, file.path(PATH, "data/pseudobulk/combined_data_pb.rds"))