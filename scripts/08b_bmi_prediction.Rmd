...
if (do_save) {
  ...
  tcga_obese_gsva <- readRDS(file.path(PATH, "data/gsva/tcga_gsva_obese.rds"))
  metabric_obese_gsva <- readRDS(file.path(PATH, "data/gsva/metabric_gsva_obese.rds"))
  tcga_ir_gsva <- readRDS(file.path(PATH, "data/gsva/tcga_ir_gsva.rds"))
  metabric_ir_gsva <- readRDS(file.path(PATH, "data/gsva/metabric_ir_gsva.rds"))
}
```
```{r}
...
## HVG
```{r}
navin_var_5k <- readRDS(file.path(DATA_PATH, "navin_2023/navin_var_avg_5000.rds"))
navin_var_5k <- mapIds(org.Hs.eg.db, keys=navin_var_5k, column="SYMBOL", keytype="ENSEMBL", multiVals="first")
combined_var_5k <- readRDS(file.path(PATH, "data/var_genes/combined_var_avg_5000.rds"))

all_5k <- intersect(navin_var_5k, combined_var_5k)
combined_data_avg_5k <- combined_data_avg[all_5k,]
combined_data_avg_data_5k <- LayerData(combined_data_avg_5k, "scale.data") %>% as.matrix %>% t %>% as.data.frame
```
...
## Random Forest model
```{r}
rf_model <- readRDS(file.path(PATH, "brca_atlas_validation/data/rf_5k_best.rds"))
```
...
```{r}
rf_test_pred <- 
  predict(rf_model, combined_data_avg_data_5k) %>%
  bind_cols(predict(rf_model, combined_data_avg_data_5k, type="prob"))
rf_test_pred$.pred_1 %>% hist(main="Histogram of Obesity Probabilities")
```
```{r}
# Adding IR data
ir_gsva <- readRDS(file.path(PATH, "data/pseudobulk/combined_data_hardy_gsva.rds"))
ir_gsva_coldata <- colData(ir_gsva) %>% as.data.frame %>% dplyr::select(c(donor,up,dn,diff))
colnames(ir_gsva_coldata) <- c("donor_id", "hardy_up", "hardy_dn", "hardy_diff")
ir_gsva_coldata$donor_id <- str_replace_all(string=ir_gsva_coldata$donor_id, "-", "_")
cellxgene_metadata <- read.csv(file.path(PATH, "data/metadata/cellxgene_metadata.csv"), row.names = 1)
cellxgene_metadata$donor_id <- str_replace_all(string=cellxgene_metadata$donor_id, "-", "_")
cellxgene_metadata <- cellxgene_metadata %>% rownames_to_column("id")
combined_metadata <- dplyr::left_join(cellxgene_metadata, ir_gsva_coldata, by = "donor_id")
# Adding obesity data
obesity_pred <- data.frame(donor_id = rownames(combined_data_avg_data_5k), obesity_prob = rf_test_pred$.pred_1)
obesity_pred$donor_id <- str_replace_all(string=obesity_pred$donor_id, "-", "_")
combined_metadata <- dplyr::left_join(combined_metadata, obesity_pred, by = "donor_id")
combined_metadata <- combined_metadata %>% column_to_rownames("id")
write.csv(combined_metadata, file.path(PATH, "data/metadata/cellxgene_metadata_metabolic.csv"), row.names = TRUE)
```
